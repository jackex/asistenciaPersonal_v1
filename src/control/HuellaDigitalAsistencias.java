 ...
 * @author juliancc
 */
public class HuellaDigitalAsistencias {
    
    DBConnection conexion = new DBConnection();
    protected DPFPCapture LECTORASISTENCIAS = DPFPGlobal.getCaptureFactory().createCapture();
    ...
    public HuellaDigitalAsistencias(RegistrarAsistencias asistencias){
    this.ASISTENCIAS = asistencias;
    }
    
    public HuellaDigitalAsistencias(){}
    
    public void start(){
        LECTORASISTENCIAS.startCapture();
        mostrarMensaje("Lector de huella activado");
    }
    
    public void stop(){
    LECTORASISTENCIAS.stopCapture();
    mostrarMensaje("Lector de huella desactivado");
    }
    
    public void mostrarMensaje(String texto){
        ASISTENCIAS.mensajeHuella.append(texto + "\n");     
    }
   ...
   /* public void dibujarHuella(Image image){
        ASISTENCIAS.LblMostrarHuella.setIcon(new ImageIcon(image.getScaledInstance(
    ...
    public void Iniciar(){
        LECTORASISTENCIAS.addDataListener(new DPFPDataAdapter(){
            @Override
        ...
        LECTORASISTENCIAS.addReaderStatusListener(new DPFPReaderStatusAdapter(){
            @Override
        ...
        LECTORASISTENCIAS.addSensorListener(new DPFPSensorAdapter(){
            @Override
        ...
        LECTORASISTENCIAS.addErrorListener(new DPFPErrorAdapter(){
            public void errorReader(final DPFPErrorEvent e){
                SwingUtilities.invokeLater(new Runnable() {
                    @Override
                    public void run() {
                        mostrarMensaje("Error " + e.getError().getErrorText());
                    }
                });
            }            
        });       
                 ...
                 // this.dibujarHuella(imagen);
                  ...
                  switch(this.RECLUTADOR.getTemplateStatus()){
                      case TEMPLATE_STATUS_READY:
                          ...
                          this.ASISTENCIAS.DETECCIONHUELLA.setEnabled(true);
                          this.ASISTENCIAS.cerrarVentana();
                          consultarHuella(this.documentousuario);                          
                          break;
            ...
            try { 
                ...
                while(RS.next()){                    
                    ...
                    if(result.isVerified()){
                       this.ASISTENCIAS.verificarIngresos(Integer.parseInt(documento));
                    }else{
                        JOptionPane.showMessageDialog(null,"No existe o no coincide el registro de huella para este usuario","Mensaje",JOptionPane.ERROR_MESSAGE);
                    }                    
                }
                PS.close();
                RS.close();
